...
**Q9**
There are `r sum(is.na(psy_stat$height))` missing data points for height


**Q10** 
There are `r sum(psy_stat$height > 173, na.rm = T)` students taller than me. That is, `r sum(psy_stat$height > 173, na.rm = T) / (nrow(psy_stat) - sum(is.na(psy_stat$height)))*100` %.
...
**Q13**
The boundaries of a 95 % confidence interval of the mean sleep rating (`r sum$mean`) is `r sum$mean -1.96*(sum$sd/sqrt(sum$n))` and `r sum$mean + 1.96*(sum$sd/sqrt(sum$n))`